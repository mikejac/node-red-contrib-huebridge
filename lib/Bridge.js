...
class Bridge extends Aggregation(Lights, Groups, Schedules, Scenes, Sensors, Rules, Configuration, Resourcelinks, Capabilities, Datastore, SSDP, RuleEngine, Emitter) {
    ...
    start() {
        ...
        this.httpServer.on('error', function(error) {
            if (!error) {
                node.warn("Bridge::httpServer(on-error): unable to start");
                node.emit('http-error', "unable to start; no error message");
                return;
            }
            ...
            errorText += " (p:" + node.dsGetHttpPort() + ")";

            node.warn("Bridge::httpServer(on-error): " + errorText);
            node.emit('http-error', errorText);
        });

        // start server
        this.httpServer.listen(this.dsGetHttpPort(), function(error) {
            if (error) {
                node.warn("Bridge::httpServer(listen): " + JSON.stringify(error));
                node.emit('http-error', error);
                return;
            }
    ...
    _debug(data) {
        var now = new Date();
        var str = 'D' + this.dsDateString() + ': ' + data;
        console.log(str);
    }
    //
    //
    ...
    _warn(data) {
        var now = new Date();
        var str = 'W' + this.dsDateString() + ': ' + data;
        console.log(str);
    }
};
/******************************************************************************************************************
