...
class RuleEngine {
        ...
        switch (t.type) {
            case 'recur':
                ...
                var job = schedule.scheduleJob(rule, function(idX, oX) {
                    ...
                    myself.debug("RuleEngine::_scheduleEngineAdd(recur): run timer, id = " + id + ", o = " + JSON.stringify(o));
            ...
            case 'recurRand':
                ...
                var job = schedule.scheduleJob(rule, function(idX, oX) {
                    ...
                    myself.debug("RuleEngine::_scheduleEngineAdd(recurRand): run timer, id = " + id + ", o = " + JSON.stringify(o));
            ...
            case 't1':
                ...
                var job = schedule.scheduleJob(runTime, function(idX, oX) {
                    ...
                    myself.debug("RuleEngine::_scheduleEngineAdd(t1): run timer, id = " + id + ", o = " + JSON.stringify(o));
     ...
     */
    sensorEngine() {
        this._sensorEngineSuncalc(true, 1);
    ...
    //
    _sensorEngineSuncalc(initial, id) {
        var myself = this;
        var idX    = id;     // make a copy of object
    ...
    _sensorEngineSetDaylightJob(id, time, state) {
        ...
        this.sensorDaylightJob = schedule.scheduleJob(time, function(idX, stateX) {       // runs at localtime
            myself.debug("RuleEngine::_sensorEngineSetDaylightJob(): id = " + id);

            var o = myself.dsGetSensor(id);
            o.state.daylight = state;
            myself.dsUpdateSensor(id, o);

            process.nextTick(() => {
                myself.emit('sensor-state-modified', id, o, '/sensors/' + id + '/state/daylight', o.state.daylight);
            });            

            // prepare for next event
            myself.sensorDaylightJob = null;
            myself._sensorEngineSuncalc(false, id);
        });
    }
    //
    //
