...
class RuleEngine {
    ...
    sensorEngine() {
        ...
        this.on('sensor-modified', function(id, o) {
            myself.debug("RuleEngine::sensorEngine(sensor-state-modified): id = " + id);

            if (id == '1') {
                myself.debug("RuleEngine::sensorEngine(sensor-state-modified): daylight");
        
                if (o.config.long != 'none' && o.config.lat != 'none') {
                    myself.debug("RuleEngine::sensorEngine(sensor-state-modified): daylight; long/lat set");
